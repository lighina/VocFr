...
class TestViewModel {
    ...
    private func saveTestRecord(result: TestResult) {
        ...
        do {
            ...
            // Track achievements
            trackAchievements(accuracy: Double(result.correctAnswers) / Double(result.totalQuestions), context: modelContext)
    ...
    /// Update WordProgress for all words in this test session
    private func updateWordProgress(context: ModelContext) {
        ...
        do {
            try context.save()
            print("✅ Updated WordProgress for \(questions.count) words")
        } catch {
    ...
    private func trackAchievements(accuracy: Double, context: ModelContext) {
        ...
        if accuracy >= 1.0 {
            ...
            if let perfectRecords = try? context.fetch(perfectDescriptor) {
                let isPerfect20 = questions.count >= 20
